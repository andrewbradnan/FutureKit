...
private enum PromiseState<T : Equatable> :  CustomStringConvertible, CustomDebugStringConvertible {
    ...
    func addExpectations(_ promise : Promise<T>,testCase : FKTestCase,testVars: PromiseTestCase<T>, name : String) { // validate promise in this state
        ...
        case let .success(expectedValue):
            ...
            let onCompleteExpecation = testCase.expectation(withDescription: "OnComplete")
            let onSuccessExpectation = testCase.expectation(withDescription: "OnSuccess")
        ...
        case let .fail(expectedError):
            ...
            let onCompleteExpecation = testCase.expectation(withDescription: "OnComplete")
            let onFailExpectation = testCase.expectation(withDescription: "OnFail")
        ...
        case .cancelled:
            ...
            let onCompleteExpecation = testCase.expectation(withDescription: "OnComplete")
            let onCancelExpectation = testCase.expectation(withDescription: "OnCancel")
...
func howMuchTimeLeft() -> String {
    ...
    if let date = _dateStarted {
        ...
        let localTime = DateFormatter.localizedString(from: timeWeWillFinished, dateStyle: .noStyle, timeStyle: .shortStyle)
...
private struct PromiseFunctionTest<T : Equatable> {
    ...
    func executeTest(_ testCase : FKTestCase, testVars: PromiseTestCase<T>, name : String) {
        ...
        let expectation = testCase.expectation(withDescription: "testFinished")
        ...
        testCase.waitForExpectations(withTimeout: maxWaitForExpecations, handler: nil)
...
extension PromiseFunctionTest {
    ...
    static func createAllTheTests(_ result:T, result2:T, promiseExecutor:Executor,futureExecutor:Executor) -> [PromiseFunctionTest] {
        ...
        let exception = NSException(name: "PromiseFunctionTest" as NSExceptionName, reason: "Reason 1", userInfo: nil)
        let exception2 = NSException(name: "PromiseFunctionTest 2" as NSExceptionName, reason: "Reason 2", userInfo: nil)
    ...
    func addExpectation(_ testCase : FKTestCase) -> XCTestExpectation? {
        ...
        case let .completeWithBlocksOnAlreadyCompleted(_,completeWillSucceed):
            if (!completeWillSucceed) {
                return testCase.expectation(withDescription: "we expect OnAlreadyCompleted to be executed")
            }
            else {
                return nil
            }
        ...
        case let .completeWithOnCompletionError(_,completeWillSucceed):
            if (!completeWillSucceed) {
                return testCase.expectation(withDescription: "we expect OnCompletionError to be executed")
            }
            else {
                return nil
            }
        ...
        for promiseE in quick_executors_list {
            for futureE in quick_executors_list {
                ...
                for (index,t) in tests.enumerated() {
                    ...
                    let c : NSArray = n.components(separatedBy: setOfCharsWeGottaGetRidOf)
                    let name = c.componentsJoined(by: "")
...
//        tests += self.testsFor(v, result2: v)
    ...
    func onPromiseSuccessVoid(promiseExecutor p: Executor, futureExecutor : Executor) {
        ...
        let completeExpectation = self.expectation(withDescription: "Future.onComplete")
        let successExpectation = self.expectation(withDescription: "Future.onSuccess")
